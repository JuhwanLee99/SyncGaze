...
interface CrosshairProps {
    color?: string;
    containerRef?: RefObject<HTMLElement | null>;  // Change this line
    circleRadius?: number;
    showCircle?: boolean;
  }
...
const Crosshair: React.FC<CrosshairProps> = ({ 
  ...
  useEffect(() => {
    const handleMouseMove = (ev: Event) => {
      ...
      if (containerRef?.current) {
        ...
        if (
          ...
          gsap.to([lineHorizontalRef.current, lineVerticalRef.current, circleRef.current].filter(Boolean), { opacity: 0 });
        } else {
          gsap.to([lineHorizontalRef.current, lineVerticalRef.current, circleRef.current].filter(Boolean), { opacity: 1 });
        }
      }
    };
    ...
    gsap.set([lineHorizontalRef.current, lineVerticalRef.current, circleRef.current].filter(Boolean), { opacity: 0 });

    const onMouseMove = (_ev: Event) => {
      renderedStyles.tx.previous = renderedStyles.tx.current = mouse.x;
      renderedStyles.ty.previous = renderedStyles.ty.current = mouse.y;

      gsap.to([lineHorizontalRef.current, lineVerticalRef.current, circleRef.current].filter(Boolean), {
        duration: 0.9,
        ease: 'Power3.easeOut',
        opacity: 1
      });
    ...
    const render = () => {
      ...
      if (lineHorizontalRef.current && lineVerticalRef.current) {
        ...
        if (circleRef.current && showCircle) {
  ...
  return (
    ...
    >
      ...
      {showCircle && (
        <div
          ref={circleRef}
          className="absolute pointer-events-none opacity-0 rounded-full"
          style={{ 
